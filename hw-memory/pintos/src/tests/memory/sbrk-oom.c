...
static int fill_memory(int step_size) {
  int count = 0;
...
static void empty_memory(int count, int step_size) {
...
static int oom_iteration(int step_size) {
...
void test_sbrk_oom(int num_iterations, int step_size) {
  ...
  for (int i = 0; i != num_iterations - 1; i++) {
    ...
    if (reached_depth != expected_depth) {
      fail("after run %d/%d, expected depth %d, actual depth %d.", i + 2, num_iterations,
